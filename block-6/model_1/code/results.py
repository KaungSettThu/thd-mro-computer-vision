...
if __name__ == '__main__':
    ...
    for ckpt, name, out, dim, bb in [
        ('checkpoints/model_triplet.pth',       'Base: Triplet Loss (ResNet18)',         'retrieval_triplet.png',      128, 'resnet18'),
        ('checkpoints/model_contrastive.pth',   'Base: Contrastive Loss (ResNet18)',     'retrieval_contrastive.png',  128, 'resnet18'),
        ('checkpoints/model1_hard_triplet.pth', 'Model 1: Hard Triplet (ResNet50)',      'retrieval_model1.png',       256, 'resnet50'),
    ]:
        if not os.path.exists(ckpt):
            print(f"Checkpoint not found: {ckpt} — skipping.")
            continue
        model = get_model(embedding_dim=dim, pretrained=False, backbone=bb).to(DEVICE)
